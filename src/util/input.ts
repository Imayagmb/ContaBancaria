...
export class Input {
    ...
    /**
 * ❓ Faz uma pergunta SIM ou NÃO (modo normal)
 *
 * 💡 QUANDO USAR:
 * - Para confirmar ações: "Deseja realmente excluir?"
 * - Para perguntas sim/não: "Você é maior de idade?"
 * - Permite resposta Y/N ou cancelar
 *
 * 📖 EXEMPLOS:
 * const confirmou = Input.keyInYN('Deseja continuar? ')
 * if (confirmou) {
 *   console.log('Usuário confirmou!')
 * } else {
 *   console.log('Usuário negou ou cancelou!')
 * }
 */
static keyInYN(pergunta: string, config?: any): boolean {
    // Detecta o encoding do console (CP850, UTF-8, etc)
    this.detectarEncoding();

    // Importa readline-sync dinamicamente
    const readlinesync = require('readline-sync');

    // Converte a pergunta para o encoding correto do console
    const perguntaConvertida = this.converterParaConsole(pergunta);

    // Retorna true se usuário apertar 'Y', false se 'N', permite cancelamento
    return readlinesync.keyInYN(perguntaConvertida, config);
}
